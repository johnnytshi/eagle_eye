...
def detect_objects(output_q, image_np, sess, image_tensor, boxes, scores, classes, num_detections):
...
def input_worker(input_q, output_q):
    ...
    while True:
        ...
        detect_objects(output_q, frame_rgb, sess, image_tensor, boxes, scores, classes, num_detections)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-src', '--source', dest='video_source', type=str,
                        default=0, help='rtsp://admin:*@192.*.*.*:554/h264/ch1/main/av_stream')
    parser.add_argument('-token', '--slack_token', dest='slack_token', type=str,
                        default=480, help='Slack Web API Token')
